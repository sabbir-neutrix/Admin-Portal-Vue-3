    ...
    <div v-for="(category, index) in categories" :key="index" class="border rounded-md overflow-hidden bg-slate-100">
        <div class="flex justify-between items-center p-3">
            <div class="w-[60%]">
                ...
                <p class="text-base md:text-xl font-semibold line-clamp-2 mb-2">Nicola Pierson</p>
                <p class="text-sm md:text-xs font-medium leading-5 mb-1 md:mb-2 line-clamp-2">{{ category.description }}</p>
                <p class="text-xs md:text-xs leading-5 line-clamp-2">{{ category.additionalInfo }}</p>
            </div>
            <div class="w-[40%]">
